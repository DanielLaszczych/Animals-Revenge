...
export default class FreePlayLevel extends Scene {
    ...
    static waveRecord: number = 0;
    protected waveCountLabel: Label;
    protected recordCountLabel: Label;
    ...
    protected newWave: boolean = false;
    protected levelSpeed: number;

    protected farmerSpawnChance: number;
    protected soldierSpawnChance: number;
    protected superSoldierSpawnChance: number;
    protected droneSpawnChance: number;
    protected robotDogSpawnChance: number;
    protected presidentSpawnChance: number;
    ...
    protected addUI(): void {
        ...
        this.waveCountLabel = <Label>this.add.uiElement(UIElementType.LABEL, "UI", {position: new Vec2(810, 30), text: "Wave " + this.currentWave});
        this.waveCountLabel.textColor = Color.WHITE
        this.waveCountLabel.font = "PixelSimple";

        this.recordCountLabel = <Label>this.add.uiElement(UIElementType.LABEL, "UI", {position: new Vec2(800, 60), text: "Record " + FreePlayLevel.waveRecord});
        this.recordCountLabel.textColor = Color.WHITE
        this.recordCountLabel.font = "PixelSimple";
        ...
        this.startWaveBtn.onClick = () => {
            if (Input.getMousePressButton() === BUTTON.LEFTCLICK) {
                ...
                if (this.currentWave >= 2 && this.timeBetweenSpawn >= 500 && this.timeBetweenSpawn ) {
                    this.timeBetweenSpawn -= 100;
                }
                if (this.totalEnemies <= 400) {
                    this.totalEnemies += 5;
                }
    ...
    protected spawnEnemy(): void{
        ...
        if(this.spawnTimer.isStopped()){
            ...
            if(this.farmerSpawnChance > 0 && randomNumber < this.farmerSpawnChance){
            ...
            else if(this.droneSpawnChance > 0 && randomNumber < this.droneSpawnChance){
            ...
            let path;
            if (randomNumber < 0.5) {
                path = [0, 1, 2, 3, 4, 5, 6, 7].map((index: number) => this.graph.getNodePosition(index));
            } else {
                path = [0, 1, 8, 9, 10, 11, 12, 7].map((index: number) => this.graph.getNodePosition(index));
            }
            ...
            this.enemyNumber++;
            if(this.enemyNumber == this.totalEnemies) {
    ...
    updateScene(deltaT: number): void {
        ...
        if (this.waveInProgress) {
            if (this.enemies.size === 0 && !this.spawningEnemies && this.healthCount > 0) {
                ...
                this.emitter.fireEvent(AR_Events.WAVE_START_END, {isWaveInProgress: false});
                this.incMoney(150);
                if (this.currentWave > FreePlayLevel.waveRecord) {
                    FreePlayLevel.waveRecord = this.currentWave;
                    this.recordCountLabel.text = "Record " + FreePlayLevel.waveRecord;
                }
                this.currentWave++;
                this.waveCountLabel.text = "Wave " + this.currentWave;
